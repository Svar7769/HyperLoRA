...
class SMAX(MultiAgentEnv):
    def __init__(
        ...
        damage_reward_multiplier=1.0,  # Multiplier for damage rewards (>1 for extra reward)
        distance_reward_scale=0.0,  # Scale for distance-based reward shaping (0 = disabled)
    ) -> None:
        ...
        self.damage_reward_multiplier = damage_reward_multiplier
    ...
    def compute_reward(self, state, health_before, health_after):
        ...
        def compute_team_reward(team_idx):
            ...
            ) * self.damage_reward_multiplier
            enemy_health_decrease_reward = jax.lax.select(
                self.use_self_play_reward, 0.0, enemy_health_decrease_reward
            )

            # Distance-based reward shaping (potential-based for curriculum learning)
            distance_reward = 0.0
            if self.distance_reward_scale > 0.0:
                # Get positions for this team and enemy team
                team_positions = jax.lax.dynamic_slice_in_dim(
                    state.unit_positions, team_start_idx, team_size, axis=0
                )
                enemy_positions = jax.lax.dynamic_slice_in_dim(
                    state.unit_positions, other_team_start_idx, enemy_team_size, axis=0
                )
                team_alive = jax.lax.dynamic_slice_in_dim(
                    state.unit_alive, team_start_idx, team_size
                )
                enemy_alive = jax.lax.dynamic_slice_in_dim(
                    state.unit_alive, other_team_start_idx, enemy_team_size
                )

                # Compute minimum distance from each alive ally to any alive enemy
                # Shape: (team_size, enemy_team_size)
                distances = jnp.linalg.norm(
                    team_positions[:, None, :] - enemy_positions[None, :, :], axis=-1
                )
                # Mask out dead units (set distance to infinity)
                distances = jnp.where(
                    team_alive[:, None] & enemy_alive[None, :], distances, jnp.inf
                )
                # Get minimum distance for each alive ally
                min_distances = jnp.min(distances, axis=1)
                # Only count alive allies
                valid_distances = jnp.where(team_alive, min_distances, 0.0)
                # Reward: negative distance (closer = higher reward), normalized by map diagonal
                map_diagonal = jnp.sqrt(self.map_width**2 + self.map_height**2)
                # Average negative normalized distance for alive units
                num_alive = jnp.sum(team_alive)
                distance_reward = (
                    jnp.where(
                        num_alive > 0,
                        -jnp.sum(valid_distances) / (num_alive * map_diagonal),
                        0.0,
                    )
                    * self.distance_reward_scale
                )
