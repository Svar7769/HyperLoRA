...
class Scenario(BaseScenario):
    ...
    def update_agent_capabilities(self, agent_capabilities):
        """
        Update agent capabilities (speed) without recreating the world.
        This allows for randomizing capabilities between episodes for generalization.

        Note: Sampling scenario doesn't use lidar sensors, so we only update speed.

        Args:
            agent_capabilities: dict with 'speed' list (optional 'lidar_range' ignored)
        """
        if agent_capabilities is None:
            return

        n_agents = len(self.world.agents)
        agent_speeds = agent_capabilities.get("speed", [1.0] * n_agents)

        # Update each agent's speed property
        for i, agent in enumerate(self.world.agents):
            agent._max_speed = agent_speeds[i]
